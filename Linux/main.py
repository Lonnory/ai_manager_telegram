            ...
            if saved_connection_id:
                try:
                    ...
                    if message.photo or getattr(message, 'video', None) or getattr(message, 'video_note', None) or getattr(message, 'voice', None) or getattr(message, 'audio', None) or getattr(message, 'document', None):
                        try: await bot.copy_message(chat_id=business_user_id, from_chat_id=message.chat.id, message_id=message.message_id, caption=caption, parse_mode=ParseMode.HTML, disable_notification=True)
                        except Exception: pass
                    else:
                        try: await bot.send_message(chat_id=business_user_id, text=caption, parse_mode=ParseMode.HTML, disable_notification=True)
                        except Exception: pass
...
async def on_edited_business_message(message: Message):
    ...
    try:
        if media_info and ":" in media_info:
            ...
            try:
                if m_type == "photo": await bot.send_photo(chat_id=business_user_id, photo=old_file_id, caption=report_text, parse_mode=ParseMode.HTML, disable_notification=True)
                elif m_type == "video": await bot.send_video(chat_id=business_user_id, video=old_file_id, caption=report_text, parse_mode=ParseMode.HTML, disable_notification=True)
                elif m_type == "video_note":
                    await bot.send_message(business_user_id, report_text, parse_mode=ParseMode.HTML, disable_notification=True)
                    await bot.send_video_note(chat_id=business_user_id, video_note=old_file_id, disable_notification=True)
                elif m_type == "voice": await bot.send_voice(chat_id=business_user_id, voice=old_file_id, caption=report_text, parse_mode=ParseMode.HTML, disable_notification=True)
                elif m_type == "audio": await bot.send_audio(chat_id=business_user_id, audio=old_file_id, caption=report_text, parse_mode=ParseMode.HTML, disable_notification=True)
                else: await bot.send_document(chat_id=business_user_id, document=old_file_id, caption=report_text, parse_mode=ParseMode.HTML, disable_notification=True)
            except Exception as e:
                await bot.send_document(chat_id=business_user_id, document=old_file_id, caption=report_text, parse_mode=ParseMode.HTML, disable_notification=True)
        else:
        ...
        if old_msg:
            ...
            try:
                if media_info and ":" in media_info:
                    m_type, old_file_id = media_info.split(":", 1)
                    try:
                        if m_type == "photo": await bot.send_photo(chat_id=business_user_id, photo=old_file_id, caption=report_text, disable_notification=True)
                        elif m_type == "video": await bot.send_video(chat_id=business_user_id, video=old_file_id, caption=report_text, disable_notification=True)
                        elif m_type == "video_note":
                            await bot.send_message(business_user_id, report_text, disable_notification=True)
                            await bot.send_video_note(chat_id=business_user_id, video_note=old_file_id, disable_notification=True)
                        elif m_type == "voice": await bot.send_voice(chat_id=business_user_id, voice=old_file_id, caption=report_text, disable_notification=True)
                        elif m_type == "audio": await bot.send_audio(chat_id=business_user_id, audio=old_file_id, caption=report_text, disable_notification=True)
                        else: await bot.send_document(chat_id=business_user_id, document=old_file_id, caption=report_text, disable_notification=True)
                    except Exception as e:
                        await bot.send_document(chat_id=business_user_id, document=old_file_id, caption=report_text, disable_notification=True)
                else:
                    await bot.send_message(business_user_id, report_text, disable_notification=True)
            except Exception as e:
                pass
